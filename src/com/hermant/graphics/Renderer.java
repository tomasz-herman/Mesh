...
public class Renderer {
    ...
    public void renderScene(Scene scene) {
        ...
        for (GameObject object : objects) {
            ...
            List<Mesh> meshes = object.getModel().getMeshes();
            List<Mesh> reversed = new ArrayList<>();
            for (Mesh mesh : meshes) {
                reversed.add(0, mesh);
            }
            for (Mesh mesh : reversed) {
    ...
    private static long orientation(Vector2i a, Vector2i b, Vector2i c) {
        return (long)(b.x-(long)a.x)*(long)(c.y-(long)a.y) - (long)(b.y-(long)a.y) *(long)(c.x-(long)a.x);
    }
    ...
    public void renderTriangle(Triangle t, Material m, Light l){
        ...
        for (p.y = minY; p.y <= maxY; p.y++) {
            for (p.x = minX; p.x <= maxX; p.x++) {
                ...
                long w0 = orientation(v1, v2, p);
                long w1 = orientation(v2, v0, p);
                long w2 = orientation(v0, v1, p);

                // If p is on or inside all edges, render pixel.
                if (w0 >= 0 & w1 >= 0 & w2 >= 0){
                    if((w0 + w1 + w2)<2)continue;
                    float f0 = (float)w0 / (float)(w0 + w1 + w2);
                    float f1 = (float)w1 / (float)(w0 + w1 + w2);
                    float f2 = 1.0f - f0 - f1;
